...
from multi_agent_framework.core.event_bus_factory import get_event_bus, reset_event_bus  # noqa: E402
from multi_agent_framework.core.event_bus_interface import Event as BusEvent, EventType  # noqa: E402
...
class FixedEventDrivenIntegrationTest(TestCase):
    ...
    def test_agent_shutdown_handling(self):
        ...
        try:
            ...
            expected_msg = f"Expected {len(agents)} shutdown acks, got {len(shutdown_acks)}"
            self.assertEqual(len(shutdown_acks), len(agents), expected_msg)
